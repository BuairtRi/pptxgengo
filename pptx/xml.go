...
func slideObjectImageToXml(slideItemObj *SlideObject, slide *SlideBaseProps, placeholderObj *SlideObject, x, y, cx, cy, imgWidth, imgHeight int, sizing *ImageSizing, rounding *bool, locationAttr string) string {
	...
	if sizing != nil && sizing.Type != "" {
		...
		sourceW, sourceH := float64(imgWidth), float64(imgHeight)
		if sizing.SourceW > 0 && sizing.SourceH > 0 {
			sourceW, sourceH = sizing.SourceW, sizing.SourceH
		}
		s.WriteString(imageSizingXml(sizing.Type, imgSizeDim{w: sourceW, h: sourceH}, imgBoxDim{w: float64(boxW), h: float64(boxH), x: float64(boxX), y: float64(boxY)}))
		imgWidth = boxW
		imgHeight = boxH
	} else {
...
func makeXmlTheme(pres *IPresentationProps) string {
	...
	if pres.Theme.HeadFontFace != "" {
		majorFont = `<a:latin typeface="` + encodeXmlEntities(pres.Theme.HeadFontFace) + `"/>`
	}
	minorFont := `<a:latin typeface="Calibri" panose="020F0502020204030204"/>`
	if pres.Theme.BodyFontFace != "" {
		minorFont = `<a:latin typeface="` + encodeXmlEntities(pres.Theme.BodyFontFace) + `"/>`
	}
...
func customizeThemeColors(themeXML string, scheme ThemeColorScheme) string {
	replacements := []struct {
		tag, current string
		value        HexColor
	}{
		{"dk1", `<a:sysClr val="windowText" lastClr="000000"/>`, scheme.Dark1},
		{"lt1", `<a:sysClr val="window" lastClr="FFFFFF"/>`, scheme.Light1},
		{"dk2", `<a:srgbClr val="44546A"/>`, scheme.Dark2},
		{"lt2", `<a:srgbClr val="E7E6E6"/>`, scheme.Light2},
		{"accent1", `<a:srgbClr val="4472C4"/>`, scheme.Accent1},
		{"accent2", `<a:srgbClr val="ED7D31"/>`, scheme.Accent2},
		{"accent3", `<a:srgbClr val="A5A5A5"/>`, scheme.Accent3},
		{"accent4", `<a:srgbClr val="FFC000"/>`, scheme.Accent4},
		{"accent5", `<a:srgbClr val="5B9BD5"/>`, scheme.Accent5},
		{"accent6", `<a:srgbClr val="70AD47"/>`, scheme.Accent6},
		{"hlink", `<a:srgbClr val="0563C1"/>`, scheme.Hyperlink},
		{"folHlink", `<a:srgbClr val="954F72"/>`, scheme.FollowedLink},
	}
	for _, replacement := range replacements {
		color := normalizedThemeHex(replacement.value)
		if color == "" {
			continue
		}
		old := `<a:` + replacement.tag + `>` + replacement.current + `</a:` + replacement.tag + `>`
		newValue := `<a:` + replacement.tag + `><a:srgbClr val="` + color + `"/></a:` + replacement.tag + `>`
		themeXML = strings.Replace(themeXML, old, newValue, 1)
	}
	return themeXML
}

func normalizedThemeHex(value HexColor) string {
	color := strings.ToUpper(strings.TrimPrefix(strings.TrimSpace(string(value)), "#"))
	if len(color) != 6 {
		return ""
	}
	for _, r := range color {
